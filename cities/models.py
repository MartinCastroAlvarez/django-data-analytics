    ...
    def search(
        ...
        if search:
            queryset: Queryset = queryset.filter(
                ...
                | Q(state__title__icontains=search)
                | Q(state__country__title__icontains=search)
            )
        return queryset
...
class City(models.Model):
    ...
    state: models.ForeignKey = models.ForeignKey(
        State,
    ...
    def __str__(self) -> str:
        ...
        String serializer.
        """
        return f"<{self.__class__.__name__}: {self.state.country.title}, {self.state.title}, {self.title}>"
